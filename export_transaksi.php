...
$pdf->SetX(15);
$pdf->Cell(35, 6, 'Nama Member', 1, 0, 'L', 1);
$pdf->SetX(50);
$pdf->Cell(30, 6, 'Nama Mobil', 1, 0, 'L', 1);
$pdf->SetX(80);
...
while ($row = mysqli_fetch_array($result)) {
    $result2 = mysqli_query($kon, "select * from tbl_pemesan WHERE id='" . $row['id_pemesan'] . "'");
    while ($row2 = mysqli_fetch_array($result2)) {
        $result3 = mysqli_query($kon, "select * from tbl_mobil WHERE id='" . $row['id'] . "'");
        while ($row3 = mysqli_fetch_array($result3)) {
            $result4 = mysqli_query($kon, "select * from tbl_merk WHERE id_merk='" . $row3['merk'] . "'");
            while ($row4 = mysqli_fetch_array($result4)) {
                $nama = $row2["nama_pemesan"];
                $mobil = $row4["nama_merk"] . " " . $row3["nama_mobil"];
                $start = $row["tgl_pinjam"];
                $end = $row["tgl_kembali"];
                $harga = "Rp" . $row["total_harga"];
